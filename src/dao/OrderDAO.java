...
public class OrderDAO extends EduSysDAO<Order, Integer>{
    ...
    final String SELECT_BY_USER_ID = "SELECT *\n" +
                                     "FROM [Order]\n" +
                                     "WHERE user_id = ?;";
    final String GET_LATEST_ORDER_DETAILS = "{CALL GetLatestOrderDetails(?)}";
...
public List<Object[]> getLatestOrderDetails(int userId) {
        List<Object[]> results = new ArrayList<>();
        try {
            ResultSet rs = jdbcHelper.query(GET_LATEST_ORDER_DETAILS, userId);
            while (rs.next()) {
                Object[] row = new Object[4];
                row[0] = rs.getString("name");
                row[1] = rs.getFloat("price");
                row[2] = rs.getInt("qty");
            ...
            throw new RuntimeException("Error executing GetLatestOrderDetails procedure", e);
        }
        return results;
    }
