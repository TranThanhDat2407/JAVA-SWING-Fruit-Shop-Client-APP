...
public class CategoryDAO extends EduSysDAO<Category, Integer>{
    ...
    public List<Category> selectBySql(String sql, Object... args) {
        ...
        return list;
         ...
         public Map<String, Object> GetCateloryItem(int Category_id) {
        Map<String, Object> GetCateloryItem = new HashMap<>();

        String sql = "EXEC [GetItemCategory] @Category_id = ?";

        try (ResultSet rs = jdbcHelper.query(sql, Category_id)) {
            if (rs.next()) {
                for (int i = 1; rs.getMetaData().getColumnCount() >= i; i++) {
                    String columnName = rs.getMetaData().getColumnName(i);
                    Object value = rs.getObject(i);
                    GetCateloryItem.put(columnName, value);
                }
            }
        } catch (Exception e) {
            System.out.println("Lỗi CateloryDAO");
            throw new RuntimeException(e);
        }

        return GetCateloryItem;
         }
}
